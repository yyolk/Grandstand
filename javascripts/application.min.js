...
$(this).closest(".filters").find(".active").removeClass("active"),$(this).toggleClass("active"),$(this).closest(".dropdown").find(".dropdown-toggle").text(a);var b="";$(".filters .active").each(function(){b+=$(this).attr("data-filter")}),f.isotope({filter:b})});for(var k=0;k<e.length;k++){var l=e[k];$.ajax({url:"/items.php",type:"GET",data:{status:l},dataType:"json",success:function(a){for(var b=0;b<a.length;b++){var d="unassigned";"~"==a[b].score&&(a[b].score="no-score"),$.isEmptyObject(a[b].assigned_to)||(d="user-"+a[b].assigned_to.id);var e={title:a[b].title,number:a[b].number,status:a[b].status,score:a[b].score,type:a[b].type,assignedToID:d,shortUrl:a[b].short_url};c.push(e)}var f={items:c},g=$("#item_template").html();template=Handlebars.compile(g),$("#items").append(template(f)),console.log(f),console.log(g)}})}b(9e3),$.ajax({url:"/people.php",type:"GET",dataType:"json",success:function(a){for(var b=0;b<a.length;b++){var c={firstName:a[b].first_name,lastName:a[b].last_name,id:a[b].id,email:a[b].email};d.push(c)}var e={users:d},f=$("#user_template").html();template=Handlebars.compile(f),$("#users").append(template(e)),console.log(e),console.log(f)}})});